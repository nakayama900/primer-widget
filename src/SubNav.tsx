...
function SubNav({actions, classList, children, label, ...rest}: SubNavProps) {
  const classes = classnames(classList, 'SubNav')
  return (
    <SubNavBase classList={[classes]} aria-label={label} {...rest}>
      <div classList={"SubNav-body"}>{children}</div>
      {actions && <div classList={"SubNav-actions"}>{actions}</div>}
    </SubNavBase>
  )
}

...
import {Component, ParentComponent, ParentProps, splitProps} from "solid-js";
...
const StyledTitle = (props)=>{
    const [child, rest]=splitProps(props,['child'])
    return　( <Box as={'h1'} class={css`
...
const tesComponent = (props)=>{
    return <div ></div>
}
const ConfirmationHeader: ParentComponent<DialogHeaderProps> = ({title, onClose, dialogLabelId}) => {
    const onCloseClick = () => {
...
const StyledConfirmationBody = styled(Box)`
  ...
  flex-grow: 1;
`
const ConfirmationBody: Component<ParentProps<DialogProps>> = ({children}) => {
